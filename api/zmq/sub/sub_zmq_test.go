...
// // the zeromq socket.
// func (c *Channeler) actor(recvChan chan<- [][]byte, options []goczmq.SockOption) {
// 	pipe, err := goczmq.NewPair(fmt.Sprintf(">%s", c.commandAddr))
...
// 	pull, err := goczmq.NewPull(c.proxyAddr)
// 	if err != nil {
// 		c.errChan <- err
// 		return
// 	}
...
// 	sock := goczmq.NewSock(c.sockType, options...)
// 	defer sock.Destroy()
