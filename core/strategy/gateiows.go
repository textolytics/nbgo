...
func GateioWsApiClient(config *RunConfig) {
	...
	for i := 0; i <= 2; i++ {
		...
		logger.Printf("testing Minimum amount: %s\n", minAmount)
		...
		logger.Printf("LastSale: %s\n", lastPrice)

	}

}

//  type  GateioWsApiCLient struct ={GateioWsApiClient(RunConfig)}

type EventGateIoWsApi interface {
	GetTicks(Instrument1 string, Instrument2 string, Instrument3 string) (TickSnapshot []string, err error)
	GetDepths(Instrument1 string, Instrument2 string, Instrument3 string) (DepthSnapshot []string, err error)
	SendOrders(Instrument1 string, Instrument2 string, Instrument3 string) error
}
...
func NewGateIoWsApi(strategyService StrategyService) EventGateIoWsApi {
	return &gateIoWsApi{strategyService: strategyService}

}

func (g *gateIoWsApi) GetTicks(Instrument1 string, Instrument2 string, Instrument3 string) (TickSnapshot []string, err error) {
	client := gateapi.NewAPIClient(gateapi.NewConfiguration())
	// Setting host is optional. It defaults to https://api.gateio.ws/api/v4
	client.ChangeBasePath(g.config.BaseUrl)
	ctx := context.WithValue(context.Background(), gateapi.ContextGateAPIV4, gateapi.GateAPIV4{
		Key:    g.config.ApiKey,
		Secret: g.config.ApiSecret,
	})
	for _, Instrument := range Instruments {
		currencyPair := Instruments[:Instrument]
		// currency := "USDT"
		cp, _, err := client.SpotApi.GetCurrencyPair(ctx, currencyPair)
		if err != nil {
			panicGateError(err)
		...
		// for ticker := range tickers[0].Last {
		// 	logger.Printf("LastSale: %s\n", ticker)

		// }

		lastPrice := tickers[0].Last
		TickSnapshot = append(TickSnapshot, lastPrice)
...
func (g *gateIoWsApi) GetDepths(Instrument1 string, Instrument2 string, Instrument3 string) (DepthSnapshot []string, err error) {
	client := gateapi.NewAPIClient(gateapi.NewConfiguration())
	// Setting host is optional. It defaults to https://api.gateio.ws/api/v4
	client.ChangeBasePath(g.config.BaseUrl)
	ctx := context.WithValue(context.Background(), gateapi.ContextGateAPIV4, gateapi.GateAPIV4{
	...
	for _, Instrument := range Instruments {
		...
		lastPrice := tickers[0].Last
		DepthSnapshot = append(DepthSnapshot, lastPrice)
...
func (g *gateIoWsApi) SendOrders(Instrument1 string, Instrument2 string, Instrument3 string) (err error) {
	client := gateapi.NewAPIClient(gateapi.NewConfiguration())
	// Setting host is optional. It defaults to https://api.gateio.ws/api/v4
	client.ChangeBasePath(g.config.BaseUrl)
	ctx := context.WithValue(context.Background(), gateapi.ContextGateAPIV4, gateapi.GateAPIV4{
